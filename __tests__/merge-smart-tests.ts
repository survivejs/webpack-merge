...
export default function mergeSmartTests(merge) {
  commonTests(merge);

  loadersKeys.forEach(loadersKey => mergeSmartTest(merge, loadersKey));
}
...
function commonTests(merge) {
  ...
  test("should not lose CopyWebpackPlugin (#56)", () => {
    ...
    const b = {
      plugins: [new webpack.optimize.OccurrenceOrderPlugin(false)]
    };
    ...
    assert.equal(merged.plugins[1].constructor.name, "OccurrenceOrderPlugin");
  });
}
...
function mergeSmartTest(merge, loadersKey) {
  ...
  test(
    "should not merge to parent if there is no include for " +
      loadersKey +
      " #68",
    () => {
      const common = {
  ...
  test(
    "should not merge to parent if there is no exclude for " +
      loadersKey +
      " #68",
    () => {
      const common = {
  ...
  test(
    "should not merge if a loader has include and string loader values for " +
      loadersKey,
    () => {
      // these shouldn't be merged because `include` is defined.
      // instead, it should prepend to guarantee sane evaluation order
  ...
  test(
    "should not merge if a loader has exclude and string loader values for " +
      loadersKey,
    () => {
      // these shouldn't be merged because `exclude` is defined.
      // instead, it should prepend to guarantee sane evaluation order
  ...
  test(
    "should not use parent include/exclude even if only loader string is specified for " +
      loadersKey,
    () => {
      const common = {
        module: {}
  ...
  test(
    "should respect second order when existing/new have conflicting orders for " +
      loadersKey,
    () => {
      const common = {
        module: {}
